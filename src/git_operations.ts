...
async function hasLocalChanges(dir: string) {
	const res = await asyncExec("git status --porcelain", {cwd: dir, encoding: "utf8"});
	return `${res.stdout}`.trim().length !== 0;
}

async function fetch(dir: string) {
	await asyncExec("git fetch", {cwd: dir, encoding: "utf8"});
}

async function pull(dir: string, currentBranch: string) {
	let stderr, stdout;
	[stderr, stdout] = await to(asyncExec("git pull", {cwd: dir, encoding: "utf8"}));
...
async function rebase(dir: string, currentBranch: string, defaultBranch: string) {
	let stderr, stdout;
	[stderr, stdout] = await to(asyncExec(`git rebase origin/${defaultBranch}`, {cwd: dir, encoding: "utf8"}));

	if (stderr) {
		await asyncExec("git rebase --abort", {cwd: dir, encoding: "utf8"});
		return false;
	}
...
async function merge(dir: string, currentBranch: string, defaultBranch: string) {
	let err;
	[err] = await to(asyncExec(`git merge origin/${defaultBranch}`, {cwd: dir, encoding: "utf8"}));
	if (!err) {
		console.log(chalk`{yellow ${currentBranch}} was merged with {magenta origin/${defaultBranch}} in {cyan ${dir}}`);
		return;
	}
	await asyncExec("git merge --abort", {cwd: dir, encoding: "utf8"});
...
export async function gitOperations(cwd: string, projectObj: Project) {
	...
	[stderr, stdout] = await to(asyncExec("git rev-parse --agrev-ref HEAD", {cwd: dir, encoding: "utf8"}));
	...
	if (!await fs.pathExists(`${dir}`)) {
		await asyncExec(`git clone ${remote} ${dir}`, {encoding: "utf8"});

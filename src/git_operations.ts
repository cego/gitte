...
async function rebase(dir: string, currentBranch: string, defaultBranch: string, log: (arg: any) => void = console.log) {
	let err: Error & pcp.Output | null, res: pcp.Output | undefined;
...
export async function gitOperations(cwd: string, projectObj: Project): Promise<any[]> {
	const logs: any[] = [];
	const log = (arg: any) => { logs.push(arg); };

...
describe("Run action", () => {
    ...
    test("Returns if project action or group is not in config", async () => {
        ...
        const res1 = await runAction({ cwd: cwdStub, config: cnf, keys: { project: "projecta", action: "start", group: "cego.dk" }, currentPrio: 0 });
        expect(res1).toBeUndefined();

        const res2 = await runAction({ cwd: cwdStub, config: cnf, keys: { project: "projectb", action: "start", group: "cego.dk" }, currentPrio: 0 });
        expect(res2).toBeUndefined();

        const res3 = await runAction({ cwd: cwdStub, config: cnf, keys: { project: "projectc", action: "start", group: "cego.dk" }, currentPrio: 0 });
        expect(res3).toBeUndefined();
    });

    test("Returns if not current prio", async () => {
        const res = await runAction({ cwd: cwdStub, config: cnfStub, keys: { project: "projecta", action: "start", group: "cego.dk" }, currentPrio: 1 });
